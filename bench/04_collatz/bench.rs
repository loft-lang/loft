...
fn collatz_len(start: i64) -> i64 {
    ...
    let mut steps = 1;
    while cur != 1 {
        if cur % 2 == 0 { cur /= 2; } else { cur = 3 * cur + 1; }
        steps += 1;
    }
    steps
}

fn longest(lo: i64, limit: i64) -> i64 {
    let mut max_steps = 0;
    let mut max_n = 0;
    for i in lo..limit {
        let s = collatz_len(i);
...
fn main() {
    ...
    let mut sink: i64 = 0;
    for r in 0..n {
        sink = sink.wrapping_add(longest(black_box(1 + (r & 1)), black_box(limit)));
    }
