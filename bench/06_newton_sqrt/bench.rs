...
fn roots(calls: i64, salt: i64) -> i64 {
    let mut acc = 0.0f64;
    for j in 0..calls {
        acc += newton_sqrt((j + 1 + salt) as f64);
    }
    acc.round() as i64
}
...
fn main() {
    let n = arg_n(10);
    let calls: i64 = 100_000;
    let t0 = Instant::now();
    let mut sink: i64 = 0;
    for r in 0..n {
        sink = sink.wrapping_add(roots(black_box(calls), black_box(r & 1)));
    }
