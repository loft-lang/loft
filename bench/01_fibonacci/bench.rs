...
//
// One OP is `fib(30)` or `fib(31)`, alternating.  The timed loop runs `--n` ops, each on an input that differs by
...
fn main() {
    ...
    let mut sink: i64 = 0;
    for r in 0..n {
        sink = sink.wrapping_add(fib(black_box(30 + (r & 1))));
    }
    let us = t0.elapsed().as_micros() as i64;
    row("fibonacci", n, us, 1, fib(30), sink);
}
