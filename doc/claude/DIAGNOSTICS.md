...
| `api-copies-collection` | advice | A `pub` function taking a record by value answers a collection of it (`pub fn pieces(d: Doc) -> vector<Piece> { d.pieces }`). A caller cannot borrow across an API, so every call copies every element, and a caller that reads it per element copies it per element — per-call work the contract forces and no rewrite can share (`@FR-R-Escape`). Reaches the library's author only. | Expose the field, or answer an index and read the elements where they live. | C · `@F106` |
| `api-redoes-per-field` | advice | A `pub` function builds a heap-owning intermediate from its parameter with a library call and answers ONE scalar or text of it (`return pa_text(pa_decode(frame), "op")`): a caller that needs three values builds it three times. Measured 11–55× against the Rust twin on the pluginabi accessors; a no-heap intermediate, a stdlib producer, an intermediate that is answered, and a function that is not `pub` stay silent. | Answer the built record once, and let the caller read its fields where it needs them — make the record, then read it. | C · `@F106` |
| `double-move` | warning | One droppable value reaches TWO owners, and each owner releases what it owns, so the value is released twice. Three shapes. (1) One value handed to two owners: two containers, or a plain whole-value copy `t = s`, which owns what it copies; it fires only where both hand-offs certainly run (@PLN139). (2) A MEMBER of a container copied into another container (`Hold { h: s.h }`, `v += [vs[0]]`, `Hold { h: t.0 }`) while the source container still owns that member: it fires where the source's release is certain — its scope end, a rebind of it, or a `return` — and stays silent when the member is overwritten first (even on one path only) or when the rebind's value reads the source (heap.md D-heap-7 family 4). (3) A PARAMETER, or a member of one, copied into a container, or a parameter's member returned (`return p.h`): the caller still owns what it passed (heap.md D-heap-7 family 2). A local that holds the caller's record (`x = p`, never written through) counts as the parameter. Shapes (2) and (3) fire only when the copied record has a release to run. A whole parameter returned, a loop variable, a `match` payload binding and a tuple member copied into a tuple are outside it. | Build a second value for the second container, or give it to one container only and read it back from there; for a member, read it where it lives, or overwrite the member after the copy so the new container is its only owner; for a parameter, build a new value, or let the caller build the container from its own. | C C · `@F106` |
...
| `constant-condition` | warning | A non-optional HEAP value (`vector`, `text`, a keyed collection) in an `if`/`while` condition. `LOFT.md` § Conversions makes a heap value falsy exactly when it is null, and a non-optional one cannot be — so the branch always runs. SCALARS are deliberately outside it: their absent value is IN-BAND and reachable from a non-optional declaration (*"integer `i32::MIN` is falsy"*), so `if d` on a plain `integer` is a genuine two-state test — measured, an `integer` holding `i64::MIN` takes the ELSE branch. The `!` spelling of the same question is `redundant-null-negation`, which asks `IntegerSpec::non_null_reads_null` where that answer is known. `LOFT_NO_CONSTANT_CONDITION` opts out. | Test what you meant — `len(v) > 0`, `s != ""` — or make the value nullable if you meant a presence test. | C C · `@F1` |
...
| `local-out-of-scope` | error | A local bound by a statement inside a block — an `if` / `else` arm, a loop body, a `match` arm, a bare `{ }` — is read after the block's `}`, where it no longer exists (`formal/binding.md` `(B-Scope)`, rustc's rule, loft#1600). Also when every arm binds it. A loop variable ends with its loop the same way; parameters live for the whole function. | Bind it before the block and assign it inside, or make the block's value the binding: `x = if c { a } else { b }` (a tuple for several). | C C · `@F27` |
| `loop-source-written` | warning | A loop body writes a place its `for` read to decide what to walk — a range bound (`for i in 0..m { m = 10 }`, `for i in 0..len(q) { q += [x] }`) or a text source (`for c in s { s = "zz" }`). The loop took its bounds and its source ONCE, before the first round (`formal/iteration.md` `(I-For)`), so the write changes neither how many rounds run nor what text is walked. A warning because a loop written to follow a moving end now computes something else (loft#1619). A write INTO the place (`w[0] = …` under `len(w)`), a sibling field, and a write through a callee are not reported. | Take the value into a local before the loop if the fixed end is what you meant; use `while` for a loop whose end moves. | C C · `@F28` |
...
| `both-receiver-deprecated` | warning | A function whose first parameter is named `both`. `self` already gives a function both call spellings, `x.f(…)` and `f(x, …)` (`formal/calls.md` (F-Recv), (F-OneBody)), so `both` names nothing `self` does not; the declaration still compiles and means exactly `self` (C123). A WARNING rather than advice by owner decision (2026-09-15), so a library's CI goes red on it and every published use is renamed — the call surface is unchanged by the rename. | Rename the parameter to `self`, and its uses in the body. | C · `@F16` |
...
| `narrow-fallback` | advice | A compound step (`x += n`) into a DECLARED narrow range whose result does not fit takes the type's DEFAULT — `x: u8 = 250; x += 10` answers `0` — and nothing at the site says so (C127). The written-out `x = x + 10` is refused at compile time, so this is the one arithmetic shape where the language cannot ask the author what an unfitting result should become. Reads THROUGH a `&` link, because `(B-Ref-Uniform)` says a `&τ` variable is used exactly like a `τ` one: `c = &o.count; c += 10` is the same step as `o.count += 10` and answers the same default, and a `&` PARAMETER carries it into the callee (@PLN167 A0 — before it, the guard stopped at the link and the step went unchecked). Quiet where the target keeps a code for its own failure (a `τ?`, an `i32`, a plain `integer`), where the following statement fuses the check (`if !x { … }`), and where the author chose the value with `x = (x + n) ?? d`. | Read the failure with `if !x { … }` as the very next statement, or choose the value with `?? d`. | C C · `@F4` `@F38` |
