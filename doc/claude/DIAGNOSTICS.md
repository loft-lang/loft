...
| `const-to-plain-parameter` | warning | A `const` value (or a view of one) is passed to a record or collection parameter that is not declared `const`; a plain heap parameter names the caller's value, so the callee could write it. | Declare the parameter `const` if the callee only reads it, or pass a local copy. Becomes an error once the shipped libraries declare their read-only parameters (C124). | M · `@F18` |
