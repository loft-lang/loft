...
| CL-9 | **Mostly fixed (loft#836, slice 1).** A loop whose body ends in ONE unconditional `yield` is now lazy on `--native` too: one iteration per advance, the cursor persisted in the coroutine struct. An infinite or early-`break`-consumed loop-generator therefore stops when its consumer does. These shapes still take the eager `ForLoopBody` buffer, so their side effects still interleave differently: more than one yield per iteration, a yield inside an `if`/`match`, a nested loop, a `continue`, a statement AFTER the yield, and **any `while` loop** (each needs a resume point one state cannot encode — axes A2–A5; slice 1 decomposes a `for` only). ⚠ An eager loop runs to its end before the first value is handed out, so an INFINITE `while` generator — `while true { …; yield x; }`, the usual shape of a behaviour — never hands one out on `--native`: it fills its buffer until the process runs out of memory (measured 2026-09-21, killed at a 2 GB cap). `LOFT_TIMEOUT` does not stop it first. A yield of a tuple / fn-ref (the `next_into` channel) or of a struct / vector also stays eager; a struct / vector pushed into the eager buffer is a per-yield SNAPSHOT in a store the generator owns (released when it is exhausted or abandoned), so the consumer reads the value as it was at the yield rather than the record's final state, and the statements after the loop run once the buffer is filled (loft#1356). Values agree throughout. | Put the `yield` last in a `for` loop body and yield a scalar or `text` — that shape is lazy on both backends; write an endless behaviour as `for _ in 0..<large bound>` rather than `while true`. Otherwise use **straight-line** yields, or fully drain the generator. Remaining slices: **[Lazy loop yields (CL-9)](#design-lazy-loop-yields-cl-9)** below. |
