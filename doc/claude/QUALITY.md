...
| `formal/binding.md` **OPEN: 4** — D-bind-43 (a vector written through a local `&` link from a named vector rebinds the link to a copy and loses the write, both backends, silently), D-bind-42 (a record written through a local `&` link from a named record shares it instead of copying, both backends, silently), D-bind-38 (a link to a text place is refused on both backends) and D-bind-39 (a link to an integer element or field stored in fewer than 8 bytes is refused on both backends — it read or wrote the wrong width silently until the refusal; its closure is a link that carries its target's width) | probing D-bind-36's repoint over every element kind (2026-09-14) | ⚠ open, measured; D-bind-42 and D-bind-43 are silent — 42 next (widen the parameter-only copy to local record links), then 43; 38 and 39 are loud (see the entries) |
...
through their `Span`).  loft#1450's `control::heap_null_test` adds one more — it reads an `if`
condition through its `Span` to tell `OpRefIsNull` / `OpVectorIsNull` from the scalar null
comparison, and reads that op's argument through its own.  The join re-measures to the row
...
by naming `Type::Function` and `Type::Reference`.  Opaque is the right column and the right
...
**Four fixes moved this row, and no opaque entry among them is a gap.**  The
