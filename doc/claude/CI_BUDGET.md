...
| **nightly 06:17 + on `src/**`,`default/**`** | `revalidate-libs` — every published lib against this loft (HARD: does it still compile and test), plus the warning **ratchet** (SOFT on existing debt, HARD on a library that was clean and now warns — `index/lib_warning_ratchet.json`) | `schedule`, `push`, `pull_request` |
