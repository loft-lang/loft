...
| **nightly 06:17 + on `src/**`,`default/**`** | `revalidate-libs` — `gate`: every published lib still compiles + tests against this loft (a break is the freeze's business). `release-ready`: can each lib be BUILT AND RELEASED as it stands — RED on any warning, **advisory, never a required check** | `schedule`, `push`, `pull_request` |
