...
| [`plans/164-activation-arena/`](plans/164-activation-arena) (`@PLN164`) | M-MH | @PLN157 (the parse row); plan 51's guards; @PLN163 (E12) | Activation arenas, adopt-at-bind, build-in-place: the compiler removes the per-call temporaries a record-returning style mints — `parse` 7.6× → ~3× without a consumer line changing.  **Not ready:** the owner reviews the plan's edge-case matrix first |
