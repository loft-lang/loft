...
| a **projection**, a FIFTH spelling | the op names a matcher lists, now including `OpGetRecord` / `OpVectorRef` (the row above) | `OpGetDbRef` — how a CAPTURE is read out of a closure record | `projection_container_var` answers `None` for every capture read, so a closure's captured value has no container to witness.  Found while fixing loft#1485 (peer stream, 2026-09-09); the collection half is split out as loft#1489 |
| a **capture**, as a PARSER FACT | `captured_names`, populated on pass 1 | the same set on pass 2, where it is **EMPTY** | a capture predicate written once reads TRUE on pass 1 and FALSE on pass 2, so a gate keyed on it fires on one pass only — the H5 two-pass contract's own failure mode, and the reason loft#1485's first cure passed its cells and exhausted the store table (peer stream, 2026-09-09) |
...
| a **WRITE through a record's collection field** | `OpAppendVector(OpGetField(m, off, _), …)` — what a matcher written from the read spelling expects | `OpNewRecord(m, <the RECORD's type>, <field NUMBER>)` for `m.v += [x]`, and `OpGetVectorNullable(OpGetField(m, off, _), …)` followed by a set through the ELEMENT for `m.v[0]?.f = 1` — an op that only READS its container, answering a place the site then writes | @PLN164 C5's site gate, where the direction is what saved it: it ALLOW-lists the read contexts instead of denying the write ones, so the first spelling declined as an unrecognised use with no rule of its own.  The second still got through, because the op is genuinely read-only and only its RESULT is not — measured on the emission, the write landed in the caller's container, reading 109 where the record form's copy holds 9.  An op answering a PLACE is now not a read context at all |
