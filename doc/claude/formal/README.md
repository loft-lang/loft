...
re-measure against the oracle line its chapter names. The two in operational.md are the
**meta** entry, `D-op-1`/`D-op-2` — there being no shared operational semantics, the
...
close); every operational chapter below inherits them. The one in layout.md is `D-layout-1`: the load-time layout gate refuses a stale store in `store_load`
