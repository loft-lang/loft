...
| [layout.md](layout.md) | 1 | **D-layout-1** (loft#1562) — no version guard on persisted bytes (#477: same types, different bytes, silently misread; `L-Sound`). **Mechanism shipped (@PLN97):** the golden byte-layout test catches a change at commit; the `.dschema` sidecar (`CorruptReason::SchemaMismatch`) detects a stale store at load, and `store_load` and the paged loaders refuse it by default (loft#700). **Residual (re-measured 2026-09-21):** the durable `store_persist_bind` does not ask the gate and misreads a stale store — closes when it does |
