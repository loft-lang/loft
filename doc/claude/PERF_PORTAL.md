...
Commit `65f8223aa` (uncommitted changes in the tree), rustc 1.97.0 (2d8144b78 2026-07-07), reference `rustc -O`, 7 samples of ~400 ms, pinned to the fastest core.
...
| shipped routines — stdlib + libraries (the `(Perf-Weight)` population) | 162 | **2.90×** | 51 | 33 | 78 |
| stdlib | 36 | **1.83×** | 21 | 7 | 8 |
| libraries | 126 | **3.21×** | 30 | 26 | 70 |
...
| **record-field** | reading and writing fields of records reached through a vector | 16 | **5.60×** | 1.30× | 22.42× `fill_polygon` | 3 | 9 |
...
| **vector-write** | element writes and fills of scalar vectors, in place | 20 | **1.50×** | 0.56× | 9.78× `canvas` | 12 | 4 |
...
| `encode_bytes` | cbor | library:cbor | **26.35** | 25.02–27.01 (noisy) | 2.00 ms | 76.1 µs | **over 3x** | a text payload appended ONE byte at a time (`buf += [value.byte_at(i) as u8]`) |
...
| `draw_bezier` | graphics | library:graphics | **9.61** | 9.58–9.63 | 19.86 ms | 2.07 ms | **over 3x** | two slice-copies of the whole explicit stack per subdivision |
| `parse` | drawing | library:drawing | **2.44** | 2.42–2.47 | 16.9 µs | 6.9 µs | over 2x | scene text to records: scan, split, records handed between helpers |
...
| routine | lane | population | × Rust | range | native | Rust | | what it stands for |
|---|---|---|---:|---|---:|---:|---|---|
| `fill_polygon` | graphics | library:graphics | **22.42** | 22.33–22.47 | 6.60 ms | 294.5 µs | **over 3x** | per scanline a walk of `vector<Coord>` reading `.px/.py`, a FRESH crossings vector and an insertion sort |
...
| `encode` | cbor | library:cbor | **11.26** | 11.25–11.29 | 17.12 ms | 1.52 ms | **over 3x** | canonical map order: since 0.1.5 each key is encoded ONCE into a flat buffer and ranked by an O(n^2) `key_lt`  |
...
| `terrain_relief_pass` | hex_terrain | library:hex_terrain | **6.14** | 6.13–6.14 | 15.38 ms | 2.51 ms | **over 3x** | the second pass reads a type through a NULLABLE record per cell |
...
| `wide_line` | drawing | library:drawing | **1.30** | 1.29–1.31 | 7.7 µs | 5.9 µs | ok | wide line through the polygon crossing table |
...
| `bone_shape_has` | hex_body | library:hex_body | **5.29** | 4.50–5.35 (noisy) | 10.97 ms | 2.07 ms | **over 3x** | the whole chain re-walked on EVERY point query |
...
| `draw_line` | graphics | library:graphics | **2.06** | 2.06–2.07 | 12.69 ms | 6.15 ms | over 2x | Bresenham calling the METHOD `set_pixel` once per step |
...
| `decode` | cbor | library:cbor | **17.84** | 17.81–17.85 | 34.95 ms | 1.96 ms | **over 3x** | one record appended per item, a fresh `Decoded` returned per node, recursion per element |
...
| `smooth` | drawing | library:drawing | **3.29** | 3.12–3.54 (noisy coarse) | 1.3 µs | 381 ns | **over 3x** | Catmull-Rom smoothing of a point list |
...
| `fronds` | drawing | library:drawing | **1.87** | 1.86–1.89 | 94.9 µs | 50.8 µs | ok | fractal fronds: records with vector fields appended |
...
| `terrain_hydrology` | hex_terrain | library:hex_terrain | **2.35** | 2.35–2.35 | 57.64 ms | 24.55 ms | over 2x | a SELECTION-scan priority flood, O(n^2) by its own comment, over seven parallel vectors |
...
| `render_lock` | drawing | library:drawing | **1.58** | 1.57–1.62 | 4.52 ms | 2.86 ms | ok | render + Lanczos resample |
| `render_marks` | drawing | library:drawing | **1.54** | 1.52–1.55 | 1.37 ms | 888.0 µs | ok | render + Lanczos resample |
| `resize` | drawing | library:drawing | **1.41** | 1.40–1.42 | 24.66 ms | 17.52 ms | ok | Lanczos resample 1024 to 256 |
| `blend_pixel` | graphics | library:graphics | **1.30** | 1.30–1.30 | 1.17 ms | 902.0 µs | ok | read-modify-write plus a `blend` call: 4 unpacks, 3 multiply-adds, a repack |
...
| `rig_world_frame3` | hex_body | library:hex_body | **5.25** | 5.22–5.26 | 17.42 ms | 3.32 ms | **over 3x** | TWELVE parallel float vectors allocated per call to carry a Rodrigues + 3x3 matmul chain |
...
| `terrain_surface_at` | hex_terrain | library:hex_terrain | **2.90** | 2.90–2.90 | 14.90 ms | 5.13 ms | over 2x | per sample a weight vector ALLOCATED, a 3x4 kernel-tap loop, and hundreds of hashed-noise evaluations |
...
| `terrain_fbm` | hex_terrain | library:hex_terrain | **1.55** | 1.55–1.55 | 18.96 ms | 12.20 ms | ok | per octave four integer hashes and a smoothstep lerp: the densest per-op kernel in the tree |
...
| `rig_read` | hex_body | library:hex_body | **1.75** | 1.74–2.00 (noisy) | 10.08 ms | 5.76 ms | ok | every word helper re-splits the line: a 12-field bone line is split ~15 times |
...
| `fill_triangle` | graphics | library:graphics | **2.89** | 2.87–2.92 | 19.70 ms | 6.81 ms | over 2x | two multiply-before-divide edge interpolations per scanline |
...
| routine | lane | population | × Rust | range | native | Rust | | what it stands for |
|---|---|---|---:|---|---:|---:|---|---|
| `canvas` | graphics | library:graphics | **9.78** | 9.75–9.87 | 2.34 ms | 239.5 µs | **over 3x** | every frame, atlas and layer allocates one through the doubling fill |
...
| `fill_rect` | graphics | library:graphics | **3.06** | 3.05–3.07 | 7.52 ms | 2.46 ms | **over 3x** | the commonest primitive: `y*width+x` and a bounds test per pixel |
...
| `lock` | drawing | library:drawing | **1.34** | 1.33–1.36 | 2.10 ms | 1.56 ms | ok | brush-lock raster |
| `composite` | drawing | library:drawing | **1.27** | 1.26–1.27 | 128.2 µs | 101.1 µs | ok | layer compositing: read, blend, write per pixel |
| `lock_curved` | drawing | library:drawing | **1.13** | 1.10–1.13 | 1.48 ms | 1.31 ms | ok | brush-lock raster along a curve |
| `index_write` | 14_stdlib_vector | stdlib | **1.03** | 1.03–1.03 †14d99b7c6 | 4.0 µs | 3.8 µs | ok | `v[i] = …` over 20k |
| `fill_star` | drawing | library:drawing | **0.71** | 0.70–0.72 | 13.9 µs | 19.6 µs | ok | polygon scanline fill, concave |
| `fill_circle` | drawing | library:drawing | **0.56** | 0.56–0.56 | 30.4 µs | 54.1 µs | ok | polygon scanline fill |
...
† measured at an earlier commit than `65f8223aa` (a partial run re-measures only what it names); the commit follows the mark.
