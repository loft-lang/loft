...
Finished (2026-09-25) — P0 done; P1 rewritten 2026-09-15 to a rule read off the line (owner); P2's report
...
`LOFT_NO_LEASE_REFUSE=1` is the drop gate's switch-off and the bisect step.  P4 and P5 are DONE
(2026-09-24): `OpCopy` runs at every copy a leasing type makes, the view-lowered ones
materialised (`formal/heap.md` D-heap-9 closed), and the reverse hand-off for a copy of what the
caller holds is removed; P6 is DONE (2026-09-25): the read-through half landed 2026-09-24, and `(H-Elide)`'s
obligation is verified on every route that elides a copy — see P6's row.  **The plan is
finished.**  Tracked as
...
| **P3** — the report becomes a compile error with the message the rule asks for; `(H-View-Drop)`; the corpus and fixtures converted | P2r | `tests/scripts` refusal cells (`@EXPECT_ERROR`) on both backends; `INTERFACES.md` § `OpDrop` rewritten | **`(H-View-Drop)` DONE 2026-09-17** — `heap.md` D-heap-11 closed, the register 3 → 2.  A view of a droppable member whose container is disturbed is refused instead of copied, raised from the walk that already refuses for `&` (`def_reshape_refusals`) with one added condition — the view's type owns a droppable — and its own message, because the `&` family's names a lost write that a plain view never had.  13 cells both backends, corpus 1590/0 changed, guards `a-view-of-a-droppable-member-stays-a-view.loft` + `parse_errors::h_view_drop_*`.  Boundary: a callee's GROWTH still copies (`disturbed: None` on the refusal walk; @PLN164 C3's separable widening).  **Still open in P3:** the D-heap-8 refusal itself (every written copy of a droppable) and the corpus/fixture conversion it needs.  **Sized 2026-09-17, both backends identical: 204 of 285 gate cells are refused by the lease rules, while only 91 are pinned as not-clean today** — so the bulk of the conversion is cells that currently compile AND release each resource exactly once, and the refusal will fail them on the rules rather than on a defect.  Across the corpus the census refuses 355 rows (265 `copy`, 52 `caller`, 38 `container`) in 29 of the 37 files that declare `OpDrop`; all 37 are `tests/scripts` guards, and P0's finding still holds that no published library, consumer or registry package declares one, so the conversion is in-repo only.  ⚠ **The census OWED that refusal one more site kind, and that debt is PAID — `0ee29253f`, 2026-09-17.**  Measured 2026-09-17, both backends: `d = v` on a `vector<H>` releases one resource TWICE (`M1 L1 D1 D1`, no disturbance needed; the field spelling `d = b.v` and a growth after the bind read `M1 M2 L1 D1 D2 D1`), and `LOFT_DROP_COPY_CENSUS` lists no row for it — the bind arm matches `Value::Set(v, Var(src))` and the parser lowers a whole-COLLECTION bind into a `__vdb_N` mint-and-fill instead, so the node never exists.  P2r's corpus counts were short by that population, and a refusal built on the census alone would have compiled the shape.  **Closed:** the census now judges `OpAppendVector(dest, <existing place>)`, which is how the parser writes a whole-collection copy, and the site kind turned out to be FOUR shapes of which three are not binds at all — `d = v`, `d = b.v`, the concat `v += w` (`M70 M71 R2 D70 D71 D71`), the self-append `v += v` (`M52 R2 D52 D52`), and `d = a + b`, which copies BOTH operands and doubles both.  So an arm keyed on the bind would still have missed them.  The arm judges the SOURCE EXPRESSION rather than the op, because one parts loop emits that node for a copy and for a fresh literal alike: `v += mkv()` and `d = mkv() + mkv()` stay `move` (a bound MEASURED, not asserted), `v += [mk()]` emits no append at all, and a field-target append (`b.v += w`) is `Uses::construct_copy`'s site and releases once.  Corpus effect: 1299 census rows before the arm and 1299 after, none of them an append — no program in the tree writes the shape, so P2r's 355 refusals stand unchanged and the corpus could never have caught this.  `CENSUS_BLIND` emptied itself (the gate reported all three cells with *"its enumeration reaches this spelling now"*), and `p_v1`/`p_v2`/`p_v3` now carry `refuse:copy:v` / `refuse:container:b`.  The record twin (`d = s`) is judged `refuse:copy:s` and releases once, as does the vector PARAMETER bind (F-ParamHeap); the copy-MANIFEST half is unaffected, since this is not a generator-emitted deep copy.  **The REFUSAL itself is BUILT, behind `LOFT_LEASE_REFUSE` (opt-in), 2026-09-17.**  The verdicts are P2r's, unchanged; what is new is where they are reported and in what words.  Raised at the census's one `emit` chokepoint — every site funnels through it, so eight arms cannot drift apart — as `error[copy-of-droppable]`, positioned at the copy's own `file:line:col`, naming the copy and the rewrite the rule asks for (*"use the value where it is, pass it, build it where it belongs, or return the owner"*) and never a later line.  Measured, with the predictions written before the run: with the switch OFF the census is byte-identical over all 38 hook files (the one apparent difference was a PID-bearing scratch path in an unrelated diagnostic, and the census is deterministic across repeated runs); with it ON, 29 files refuse and the other 9 stay clean, `registry.loft` reports exactly its 8 sites and the other five sqldb drivers plus `dropscope` report none, and both backends name identical lines (the verdict is decided after the scope pass, before either generates).  355 refusal rows become 348 messages: the six per-file deltas are de-duplication by (line, column, variable, name), and no two refusals anywhere in the corpus share a site while differing in REASON, so that key collapses only genuine duplicates — a `bind` into a temp and the `place` of that same temp are one copy seen twice.  ⚠ **No message may name a compiler temp**, since a refusal can reach only a `__lift_N` (`refuse:container:__lift_2`, four sites in `1506`): `Frame::author_name` walks the view temps back to what they stand for and, where nothing the author wrote is reachable, the sentence names the TYPE instead — 0 of 348 messages name one.  `Refusal::Later` is skipped explicitly, because liveness decides an elision and never validity.  Guards: `tests/lease_refuse.rs`, subprocess cells because `code!` is in-process and a `OnceLock` switch cannot be both on and off in one binary — the refusal, the six shapes the rule calls legal, both backends, the no-temp-name property, and a switch-OFF control so the gate is pinned and not merely the error.  The code ceremony landed with it: `CODES` + trigger, the three `e1_code_set` runners arming the switch, and the DIAGNOSTICS.md row |
| **P4** — `OpCopy`: signature check (mirror `check_drop_signature`), synthesized cascade (mirror `synth_drop_cascades`), a call at every copy site on both backends | P1 | drop gate re-baselined on the lease oracle, both backends, `LOFT_POISON`; the matrix's `OpCopy` count per cell | **First half DONE 2026-09-24**: the signature check, `OpCopyAll` through the drop cascade's own walks (`Hook`), the calls inserted after the scope pass at the census's sites (`use_analysis::lease_calls`), and `copy_moves_drop_from` moving no release for a leasing copy; `tests/copy_lease.rs`.  **Second half DONE the same day**: the view-lowered copies materialised where the type leases — `p = p` through the rebind hoist, a tuple item into the tuple's storage, a `return` of a parameter or member into the buffer (per arm for a join), a whole collection through `OpCopyTail`; an elided copy is `(H-Elide)`'s.  29 cells on both backends |
| **P5** — remove the machinery that moved a release across a copy, one decider at a time | `scopes::copy_moves_drop_from`, the per-path hand-off flags, the caller-record mark | drop gate and the matrix unchanged after each removal; `introspect_diff` names exactly the removed ops | **DONE 2026-09-24.** REMOVED: the reverse hand-off of a copy of what the CALLER holds — `copy_moves_drop_from`'s parameter / caller-record branch, and its twin for a parameter's tuple member in `copy_record_handoff`; the drop gate moved exactly the 15 Refused cells that exercise it (to DOUBLE, run with the refusal off), no accepted cell and no #1569 matrix cell changed.  KEPT, each measured: the owned-source hand-off and the per-path flags, which ARE `(H-Move)` and `(H-Spent)`'s per-path release; the field hand-off (`LOFT_NO_FIELD_HANDOFF=1` also doubles the ACCEPTED `p_t1`, `mk_s(130).h.id` — it carries the compiler's own read-through copy, so it goes when P6 lowers a read-through as a view); the caller-record mark, which the refusal itself reads.  The `double-move` warning is now reachable only under `LOFT_NO_LEASE_REFUSE` (`read-after-move` refuses its shape first) — a diagnostic, left for a separate decision. Was: Open.  ⚠ **This phase's verify is why P3 must NOT convert the gate's cells.**  A cell that stops compiling measures no release, so converting the 204 the rules refuse would leave P5 with nothing to hold its removals against — the instrument and the thing it measures would be retired in the same change.  `run_cell` sets no environment, so the gate runs with the refusal OFF and is unaffected today; at the flip it takes an explicit opt-out instead.  The conversion is therefore the 29 corpus run-programs and the `registry` fixture's 8 sites, never the gate |
| **P6** — `(H-Elide)`: an elided copy skips its `OpCopy` and the matching drop | C86, `alias-where-correct.md`, `src/lease.rs`'s liveness pass | matrix cells where elision fires count hooks consistently; `LOFT_LINK_WIDEN` on and off agree on every observable value | **Read-through half DONE 2026-09-24**: a read through a member of a call result (`mk_s(130).h.id`), where the member owns a droppable, binds the CALL's record and reads the member where it lives — no copy, one whole release (`p_t1` independent of the field hand-off).  That freed the field hand-off entirely, and it was REMOVED: the lift skip table, `return_copy_out`'s `path_skip`, the per-arm `Member` skip, the `OpDropAllExcept` synthesis and `LOFT_NO_FIELD_HANDOFF`.  It had been WRONG for a leasing copy — the owner skipped a member the copy leased, so `r = mk_s(1).h` and `return s.h` each lost one release (measured `C1 R101 … D101` with no `D1`); `tests/copy_lease.rs` pins both.  The drop gate moved exactly 16 Refused cells.  **DONE 2026-09-25 — the obligation verified, not an optimisation built.**  `(H-Elide)` is permissive (*the compiler MAY elide*; neither hook may rely on running for an elided copy), so what P6 owes is that every route which DOES elide skips the hook and the drop together.  Measured over every route: the borrow tiers, the `LOFT_LINK_WIDEN` transparent-link widening, a parameter copy, a chain, a returned member and a copy in a callee — both backends, widening off and on.  Where the copy is kept, `OpCopy` runs and each structure drops once; where it is elided (a read through a parameter's member), neither hook runs for it; the widening elides NO leasing copy and changes no trace.  Guard `copy_lease::an_elided_copy_skips_its_hook_and_its_drop_together`, five cells with hand-written traces × 2 backends × widening off/on.  ⚠ A first reading over the 72 corpus files that declare `OpDrop` said "agrees" and was VACUOUS for this rule: no corpus script declares `OpCopy`, so none has a leasing copy to elide — the cells had to be written.  **Not built, deliberately:** the general liveness elision (more copies elided) and the read-through of a member that owns nothing.  Both are optimisations the rule allows and nothing needs: P0's census still holds — no published library, consumer or registry package declares `OpDrop` or `OpCopy` — so there is no copy anywhere whose cost would repay the analysis.  They belong to a performance plan the day a leasing type ships. |
