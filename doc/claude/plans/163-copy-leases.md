...
`LOFT_NO_LEASE_REFUSE=1` is the drop gate's switch-off and the bisect step.  P4's first half is DONE
(2026-09-24): `OpCopy` runs at every copy the census can follow with a statement, and the copies
the lowering implements as a VIEW stay refused (`formal/heap.md` D-heap-9).  Tracked as
