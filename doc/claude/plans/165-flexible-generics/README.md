...
BUILT too (C1–C4; `G-Key` written).  Arc D in progress: D1–D7 built (a generic struct is
declared, named as an instance, a literal infers its instance, a generic function takes
one, it has methods, and it may name itself — or a generic struct declared below it).**
