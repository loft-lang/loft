...
|---|---|---|---|
| **A1** — wire `parse_user=` and `scopes=` into `LOFT_TIMING` | `src/main.rs:8789`, `:8956`; `src/compile.rs:64-121` | the three existing fields byte-identical; new fields present and summing to within 10 % of measured total — a mis-wired timer reporting 0 or double-counting fails | Open |
| **A2** — front-end bench corpus + harness (cold / warm / edit-loop, both backends) | new `bench/frontend/` | harness must report an **injected** slowdown behind an env var; every corpus input hash-distinct per run, printed — a harness whose input stops varying is the failure this guards | Open |
