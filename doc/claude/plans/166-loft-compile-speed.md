...
**Every sub-arc shipped (A1–A3, B1–B3, C1), plus B4, B5 and B6 — the hot spots A3 attributed, cut: 35 % of a compile's instructions in B4, a further 12 % and a third of its allocations in B5, and B6 took the leads B5 named (the definition index keyed by name and answering literal names by address, the bytecode tables on Fx, a whole-body clone in the scope pass) for a further 5.8 %, −46.6 % cumulative.** What follows under *Why now* is the state at filing. loft's
...
| **B5** — the file name a position carries is shared, not owned | `src/lexer.rs` (`Position.file: Arc<str>`, `no_file()`) and the 70 sites that build or read one | the same falsifier as B4: every corpus dump byte-identical to the pre-change binary; the ratchet never grows; callgrind Ir on the large corpus | **Shipped 2026-09-26.**  `Position { file: String }` was cloned per token and per operator (`LexResult::clone` 480 040 times, `Lexer::cont` 202 816, `parse_operators` 684 206 on the large corpus), each a heap copy of a name that never changes within a file.  `Arc<str>` makes each a refcount bump; `Arc<str>::default()` is backed by a static, so a position with no file allocates nothing either — the ratchet's own two-runs-agree check caught the first draft, a `LazyLock` that minted the shared empty name once and read one allocation more on the first run.  **Large corpus 3 870 M → 3 396 M Ir (−12.2 %)**; ratchet re-pinned tiny 491 038 → 335 537 (−31.7 %), medium 1 723 159 → 1 172 738 (−31.9 %).  Dumps: 1 857 of 1 860 byte-identical; the other three (`157-null-buffer-hoist`, `177-reclaim-early-return`, `a-non-escaping-vector-local-is-a-caller-buffer`) vary between two runs of the SAME binary, before and after alike — the order of three `OpFreeRefIfDistinct(v, __ref_N)` at a return, a run-to-run permutation that predates this plan (loft#1685 — found by this falsifier, fixed in the same arc: `Function::rebind_orig` was a `std` hash map iterated to emit the releases; a `BTreeMap` now, and `tests/dump_determinism.rs` dumps each of the three files six times and demands one output, falsified against the pre-fix binary at 3 / 2 / 2 distinct dumps) |
| **B6** — the leads step f named, cut | `src/data.rs` (`DefIndex`: the index keyed by name, `get_or_std`, the address-keyed memo), `src/state/mod.rs` (`State.stack` / `vars` / `types` / `calls` / `library_names` on Fx), `src/parser/mod.rs` (`force_tret`, `work_buffer_promoted`), `src/scopes.rs` (`compute_intervals` reads the body in place) | the same falsifier as B4/B5 after EACH step: every corpus dump byte-identical, the ratchet never grows, callgrind Ir on the large corpus; plus `def_index_tests`, whose memo test must FAIL a memo that ignores an insert or a reused buffer | **Shipped 2026-09-26** — four steps, each measured: the index keyed by NAME so the stdlib fallback is one probe (−1.8 %), the bytecode tables on Fx (−1.3 %; the `HashMap<u32, u16>` step f attributed to `patch_tret_callers` was `State`'s), `def_nr` remembering literal names by address in lock-free slots (−1.9 %), one whole-body clone in `scopes::check` replaced by a split borrow (−0.9 %).  **Large corpus 3 396 M → 3 198 M Ir (−5.8 %; −46.6 % cumulative)**; ratchet re-pinned tiny 333 674, medium 1 150 895.  Two leads tried and withdrawn, with why, in PERFORMANCE.md § F1 |
