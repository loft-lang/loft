...
```rust
pub fn len(both: const vector) -> integer
```

Number of elements in the vector. Use in loop bounds: for i in 0..v.len().

```rust
pub fn len(both: const sorted) -> integer
```
...
```rust
pub fn len(both: const hash) -> integer
```

Number of elements in a hash collection.

```rust
pub fn size(both: const hash) -> integer
```
...
```rust
pub fn len(both: const spatial) -> integer
```

Number of elements in a spatial (radix / Morton tree) collection.

```rust
pub fn len(both: const trie) -> integer
```
...
```rust
pub fn exists(both: const File) -> boolean fs#read
```
